...
// Get a value for a given key (from a relation)
string getValue(Way *wayPtr, map<string, int> *mapPtr, vector<string> *strListPtr, string *strPtr) {
	if (mapPtr->find(*strPtr) == mapPtr->end()) { return ""; }
	uint keyIndex = mapPtr->find(*strPtr)->second;
	for (uint i=0; i<wayPtr->keys_size(); i++) {
		if (wayPtr->keys(i)==keyIndex) {
			return strListPtr->at(wayPtr->vals(i));
		}
	}
	return "";
}
